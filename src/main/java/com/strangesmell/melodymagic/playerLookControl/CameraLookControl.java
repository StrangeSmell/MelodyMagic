...
public class CameraLookControl implements Control {
    protected final Entity camera;
    ...
    public CameraLookControl( Entity camera,DeltaTracker deltaTracker) {
        this.camera = camera;
    ...
    public void setLookAt(double x, double y, double z) {
        this.setLookAt(x, y, z, 0.5f, 0.5f);
    }
    ...
    public void tick() {
        ...
        this.getYRotD().ifPresent(p_287447_ -> setRotation(this.camera,this.rotateTowards(camera.getYRot(), p_287447_, this.yMaxRotSpeed)));

        this.getXRotD().ifPresent(p_352768_ -> this.camera.setXRot(this.rotateTowards(camera.getXRot(), p_352768_, this.xMaxRotAngle)));

    }

    public static void setRotation( Entity camera,float f){
        camera.setYBodyRot(f);
        camera.setYHeadRot(f);
    ...
    protected Optional<Float> getXRotD() {
        double d0 = this.wantedX - this.camera.getX();
        double d1 = this.wantedY - this.camera.getEyeY();
        double d2 = this.wantedZ - this.camera.getZ();
    ...
    protected Optional<Float> getYRotD() {
        double d0 = this.wantedX - this.camera.getX();
        double d1 = this.wantedZ - this.camera.getZ();
    ...
    protected float rotateTowards(float from, float to, float maxDelta) {
        ...
        maxDelta = (float) Math.log(Math.abs(f)+1);
        float f1 = Mth.clamp(f, -maxDelta, maxDelta);
        return Mth.lerp(partialTick, from,from+ f1);
    }
